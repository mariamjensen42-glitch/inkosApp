...
object AgentSystemPrompt {
    ...
    private fun commonOutputRules(isZh: Boolean): String {
        return if (isZh) {
            """
            ## 输出要求
    ...
    fun buildChatPrompt(isZh: Boolean): String {
        return if (isZh) {
            """
            你是 InkOS 普通聊天助手。
    ...
    fun buildBookCreatePrompt(isZh: Boolean, confirmed: Boolean): String {
        return if (!confirmed) {
            if (isZh) {
                """
    ...
    fun buildShortPrompt(isZh: Boolean, confirmedIntent: String?): String {
        return when (confirmedIntent) {
            "short_run" -> {
                if (isZh) {
                    """
    ...
    fun buildPlayPrompt(isZh: Boolean, confirmedStart: Boolean, playWorldExists: Boolean): String {
        return if (confirmedStart) {
            if (isZh) {
                """
            ...
            if (isZh) {
                ...
                if (用户明确给了长期规则)，把它们原样提炼进 playStart.worldContract：时间尺度如何按动作变化并同步世界、角色是否自主行动、物件/线索/关系/装备/身份有什么语义、哪些事禁止或有代价。用户没说就留空，不要擅自加等级、数值、RPG 面板或固定每回合时间。
                if (用户明确给了配图规则)，把它们提炼进 playStart.visualContract：图片如何表达物件、关系、证据、装备或世界规则。用户没说就留空，不要擅自加绿蓝紫橙边框、游戏 UI 或数值。
                只把用户说过的事写成事实；不要为了让确认卡更完整而补具体年限、关系程度、修行经历、身份履历或世界规则。用户说“刚入门”就保持刚入门，不要扩写成“入门三年”；不确定的具体事实写成待定或省略。
    ...
    fun buildEditPrompt(bookId: String?, isZh: Boolean): String {
        val name = bookId ?: ""
        return if (isZh) {
            """
            你是 InkOS 外部编辑助手。当前入口只处理用户明确要求的内容修改。
            
            ${if (bookId != null) "当前书籍：$name" else "当前没有绑定书籍；如果用户没有明确文件或作品上下文，只能先询问。"}
            ...
            ${if (bookId != null) "Active book: $name" else "No book is bound; ask for the file or project context before editing."}
    ...
    fun buildBookPrompt(bookId: String, isZh: Boolean): String {
        return if (isZh) {
            """
    ...
    fun buildAgentSystemPrompt(
        bookId: String?,
        language: String,
        sessionKind: String = if (bookId != null) "book" else "chat",
        confirmedAction: Boolean = false,
        playWorldExists: Boolean = false,
        confirmedIntent: String? = null
    ): String {
        val isZh = language == "zh"
        return when (sessionKind) {
            "book-create" -> buildBookCreatePrompt(isZh, confirmedAction)
            "short" -> buildShortPrompt(isZh, confirmedIntent)
            "play" -> buildPlayPrompt(isZh, confirmedAction, playWorldExists)
            "edit" -> buildEditPrompt(bookId, isZh)
            "book" -> {
                if (bookId != null) {
                    buildBookPrompt(bookId, isZh)
                } else {
                    buildChatPrompt(isZh)
                }
            }
            else -> buildChatPrompt(isZh)
        }
    }
}
